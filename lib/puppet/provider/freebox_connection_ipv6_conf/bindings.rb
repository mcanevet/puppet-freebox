...
Puppet::Type.type(:freebox_connection_ipv6_conf).provide(:bindings) do
  ...
  def self.instances
    FreeboxApi::Configuration::Connection::IPv6.getConfig(session)
    .select { |k,_| %w[ipv6_enabled delegations].include? k }
    .collect { |k, v|
      v = :true if v == true
      v = :false if v == false
      new(
  ...
  def value=(v)
    ...
    FreeboxApi::Configuration::Connection::IPv6.updateConfig(self.class.session, {@resource[:name] => val})
