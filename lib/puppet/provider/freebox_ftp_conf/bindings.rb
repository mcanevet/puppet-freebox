...
Puppet::Type.type(:freebox_ftp_conf).provide(:bindings) do
  ...
  def self.instances
    FreeboxApi::Configuration::Ftp.getConfig(session)
    .select { |k,_| %w[enabled allow_anonymous allow_anonymous_write password].include? k }
    .collect { |k, v|
      v = :true if v == true
      v = :false if v == false
      new(
  ...
  def value=(v)
    ...
    FreeboxApi::Configuration::Ftp.updateConfig(self.class.session, {@resource[:name] => val})
